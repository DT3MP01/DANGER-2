    ...
    public void LoadFileRoom(SaveRoom save)
    {
        ...
        for (int i = 0; i < save.floorPositions.Count; i++) {
           ...
           GameObject floorLevel= Instantiate(SaveParameters[save.floorName[i]], 
            ...
            FurnitureData furnitureObjects=save.furnitureObjects[i];
            for(int j=0; j<furnitureObjects.positionFurniture.Count; j++){
               GameObject furnitureLevel = Instantiate(SaveParameters[furnitureObjects.prefabName[j]],
                ...
                WallData wallObjects=furnitureObjects.wallObjects[j];
            ...
            if (Physics.Raycast(rayCast.origin, rayCast.direction, out rayCastHit, rayCastMax))
            {
                ...
                if(rayCastHit.transform.tag == "Cube")
                {
                    ...
                    if ((prefab != null) && (prefab.transform.tag == "Wall" || prefab.transform.tag == "Window" || prefab.transform.tag == "Door") && dif == new Vector3(0, 1.0f, 0))
                    {
                        ...
                        {
                            if (child.tag == prefab.transform.tag || child.tag=="Untagged")
                            {
                                child.gameObject.SetActive(true);
                            }
                            else
