    ...
    for (int j = 0; j < width; j++) //verticales
    {
        for (int k = 0; k < length; k++) //horizontales
        {
            ...
            else if (grid[j, k] != 1 && remWidth == 0) 
            {
                Debug.Log("------------grid[j,k]==1 && remWidth != 0---------------");
                if (remLength == 0) 
                {   hemos terminado de introducir el objeto,CREAMOS EL OBJETO en la posici�n de la casilla+coordenadas reales en juego, restablecemos remX y remY y probamos a partir de prevX, break
